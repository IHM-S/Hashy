...
// To ensure we have a hashmap which can be used with any kind of hashing
// function and that we have a comparison function and destruction function for
// complex objects. The hash function will not involve the compression
// component, you will need to compress the hash to fit your implementation. If
// the hash, cmp or destruction function pointers are NULL, this function should
// return NULL.
struct hash_map* hash_map_new(size_t (*hash)(void*),
    ...
    if (key_destruct == NULL || value_destruct == NULL || hash == NULL ||
        cmp == NULL) {
        return NULL;
    }
    ...
    hash_m->item_map = (struct hash_item**)calloc(
        hash_m->map_capacity, sizeof(struct hash_item*));  // all nullptr
...
void hash_map_put_entry_move(struct hash_map* map, void* k, void* v) {
    ...
    if (new_n_item > 0.75 * map->map_capacity) {
        ...
        struct hash_item** new_item_map =
            (struct hash_item**)calloc(new_capacity, sizeof(struct hash_item*));
...
// If the key exists with the hashmap, it will be removed as well as a the
// value, if the key does not exist, nothing will be removed.
